...
[[ "$VERBOSE__RC" == true ]] && echo "$(date +%H:%M:%S)     ↳ […ode/…/_aliases--tools.sh] hello!"
...
alias gfp2='git fetch && git pull --recurse-submodules --rebase --autostash'
...
alias onn='npm install --global corepack iterm2-tab-set yarn yalc @ast-grep/cli && corepack enable && yarn global add bolt'
